...
serverless invoke local -f get_items_popularity -p ~/BoilerX/backend-api/mocks/search-items/success-hight-price-range.json > $outFile

echo "test 8: get items popularity (price in range): expect to succeeed"
